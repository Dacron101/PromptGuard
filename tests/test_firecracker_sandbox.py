...
class TestFirecrackerSandbox(unittest.TestCase):
    ...
    def test_run_virustotal_scan_skipped_without_key(self, mock_run):
        ...
        result = sandbox._run_virustotal_scan_file("/tmp/pkg_download/requests-2.31.0.whl", "requests")
    ...
    def test_run_virustotal_scan_injects_key_in_command(self, mock_run):
        ...
        mock_run.return_value = MagicMock(
            returncode=0, stdout='{"status": "clean"}', stderr=""
        )

        sandbox = FirecrackerSandbox(virustotal_key="test-vt-key-123")
        result = sandbox._run_virustotal_scan_file(
            "/tmp/pkg_download/requests-2.31.0.whl", "requests"
        )

        self.assertIn('{"status": "clean"}', result)
        # Check that VT key was embedded in the SSH command
        scan_call_args = mock_run.call_args[0][0]
